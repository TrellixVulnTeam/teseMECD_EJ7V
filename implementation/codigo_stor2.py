...
class MyVisionTextModel(CLIPModel):
    ...
    #Mudei o average pooling (ou seja, a linha que dizia “aux = aux.mean(dim=1)” 
    #para deixar considerar tokens que estão masked, no caso de eles existirem. 
    #Neste caso, a média que é feita no average pooling não usa todos os tokens da sequência, 
    #mas sim apenas aqueles que não estão masked.
